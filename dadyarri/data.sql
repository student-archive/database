...
insert into "group" ("speciality_id", "group_name", "university_id")
    values ((select "id"
                 from "speciality"
                 where "speciality_name" = 'Информационные системы и технологии'
                 limit 1), 'ИСТ-120', (select "id" from "university" where "university_name" = 'ВлГУ' limit 1))
         , ((select "id"
                 from "speciality"
                 where "speciality_name" = 'Программная инженерия'
                 limit 1), 'ПРИ-120', (select "id" from "university" where "university_name" = 'ВлГУ' limit 1))
         , ((select "id"
                 from "speciality"
                 where "speciality_name" = 'Вычислительная техника'
                 limit 1), 'ВТ-120', (select "id" from "university" where "university_name" = 'ВлГУ' limit 1))
         , ((select "id"
                 from "speciality"
                 where "speciality_name" = 'Прикладная информатика'
                 limit 1), 'ПИ-120', (select "id" from "university" where "university_name" = 'ВлГУ' limit 1))
         , ((select "id"
                 from "speciality"
                 where "speciality_name" = 'Управление системами'
                 limit 1), 'УС-120', (select "id" from "university" where "university_name" = 'ВлГУ' limit 1))
         , ((select "id"
                 from "speciality"
                 where "speciality_name" = 'Математика и компьютерные науки'
                 limit 1), 'МКН-120', (select "id" from "university" where "university_name" = 'ВлГУ' limit 1))
         , ((select "id"
                 from "speciality"
                 where "speciality_name" = 'Радиотехника'
                 limit 1), 'Р-120', (select "id" from "university" where "university_name" = 'ВлГУ' limit 1))
         , ((select "id"
                 from "speciality"
                 where "speciality_name" = 'Биология'
                 limit 1), 'Б-120', (select "id" from "university" where "university_name" = 'ВлГУ' limit 1))
         , ((select "id"
                 from "speciality"
                 where "speciality_name" = 'Приборостроение'
                 limit 1), 'П-120', (select "id" from "university" where "university_name" = 'ВлГУ' limit 1))
         , ((select "id"
                 from "speciality"
                 where "speciality_name" = 'Информационная безопасность'
                 limit 1), 'ИБ-120', (select "id" from "university" where "university_name" = 'ВлГУ' limit 1));
...
insert into "user" ("group_id", "account_id", "role_id", "sex_id", "first_name", "last_name", "avatar_link")
    values ((select "id" from "group" where "group_name" = 'ИСТ-120' limit 1), (select "id" from "account" order by random() limit 1), (select "id" from "role" where "role_name" = 'Студент' limit 1), (select "id" from "sex" where "sex_name" = 'Мужской' limit 1), 'Иван', 'Петров', 'https://www.google.com/url?sa=i&url=https%3A%2F%2Fzoo.dp.ua%2Fafrikanskij-karlikovyj-ezhik-osobennosti-porody-i-soderzhaniya%2F&psig=AOvVaw2Y8bZX448BRvER0nnTF3FC&ust=1649007025458000&source=images&cd=vfe&ved=0CAsQjRxqFwoTCKDG6oP09fYCFQAAAAAdAAAAABAD')
...
insert into "subject" ("group_id", "subject_name", "semester")
    values ((select "id" from "group" order by random() limit 1), 'Интерактивные графические системы', 5)
         ...
         , ((select "id" from "group" order by random() limit 1), 'Администрирование и безопасность программно-информационных систем', 6)
