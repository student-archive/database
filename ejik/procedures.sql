...
begin
    ...
    insert into "attachment" ("type_id", "attachment_link", "author", "checksum")
        values ((select "type_id" from "attachment" where "checksum" = 'тест' limit 1), (select "attachment_link"
                                                                                             from "attachment"
                                                                                             where "checksum" = 'тест'
                                                                                             limit 1), (select "author" from "attachment" where "checksum" = 'тест' limit 1), (select "checksum" from "attachment" where "checksum" = 'тест' limit 1));
...
begin
    ...
    insert into "quiz" ("subject_id", "quiz_name", "quiz_description", "questions_amount")
        values ((select "id" from "subject" where "subject_name" = 'Статьи' limit 1), 'Статьи это хорошо', 'Статьи это хорошо', 1);
    insert into "question" ("quiz_id", "question_text", "correct_answers_amount", "total_answers_amount")
        values ((select "id" from "quiz" where "quiz_name" = 'Статьи это хорошо' limit 1), 'нужно писать статьи', 1, 2);
    insert into "quiz_variant" ("question_id", "quiz_variant_text", "is_correct")
        values ((select "id" from "question" where "question_text" = 'нужно писать статьи' limit 1), 'Пишу', true);
    insert into "quiz_variant" ("question_id", "quiz_variant_text", "is_correct")
        values ((select "id" from "question" where "question_text" = 'нужно писать статьи' limit 1), 'не Пишу', false);
end
$$ language "plpgsql";
...
begin
    ...
    insert into "attachment" ("type_id", "attachment_link", "author", "checksum")
        values ((select "type_id" from "attachment" where "checksum" = 'Статьи' limit 1), (select "attachment_link"
                                                                                               from "attachment"
                                                                                               where "checksum" = 'Статьи'
                                                                                               limit 1), (select "author" from "attachment" where "checksum" = 'Статьи' limit 1), (select "checksum" from "attachment" where "checksum" = 'Статьи' limit 1));
